...
class Forcing:
    # from ideal test
    attributes = {"history": "Dec 20 00:00:00 2021"}
    dataDir = '/net/venus/kenes/user/amorrison/icar/forcing/'

    def __init__(self, sealevel_pressure=100000.0,
                 rh=0.9, u_val=0.5, v_val=0.5, w_val=0.0,
                 water_vapor_val=0.001, theta_val=300.0, 
                 height_value=500, dz_value=500.0):
        ds = xr.open_dataset(dataDir + 'boundary_conditions_new_test.nc')
        nx = len(ds.lon); ny = len(ds.lat); nz = len(ds.plev); nt = len(ds.time);
        dy = np.int(len(ds.lat)/181)
        dx = np.int(len(ds.lon)/360)
        print(nx, ny, nz, dx, dy)
